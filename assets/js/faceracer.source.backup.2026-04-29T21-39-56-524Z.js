...
let gameState = {
    ...
    ground: null,  // Ground plane reference
...
function createTree(x, z) {
    ...
    const scale = 0.8 + Math.random() * 0.4; // 0.8x to 1.2x scale
    ...
    if (treeType === 0) {
        ...
        // Round tree
        const trunkGeometry = new THREE.CylinderGeometry(0.35, 0.45, 2.5, 8);
        const trunkMaterial = new THREE.MeshStandardMaterial({ color: 0x654321 });
        const trunk = new THREE.Mesh(trunkGeometry, trunkMaterial);
        trunk.position.y = 1.25;
        tree.add(trunk);

        const leavesGeometry = new THREE.SphereGeometry(1.8, 8, 8);
        ...
        // Bush tree (short and wide)
        const trunkGeometry = new THREE.CylinderGeometry(0.4, 0.5, 1.5, 8);
...
function updateGame() {
    ...
    gameState.distance += gameState.speed / 60; // Update distance
    car.position.z = -gameState.distance; // Move car along Z axis
    
    // Camera follows car
    camera.position.z = car.position.z + 12; // Keep camera behind car
    
    // Make ground follow car
    if (gameState.ground) {
        gameState.ground.position.z = car.position.z - 200; // Keep ground ahead of car
    }
    
    // Create trees ahead of car
    const treeSpawnZ = car.position.z - 200; // Spawn trees 200 units ahead
    if (Math.random() < 0.1) { // 10% chance per frame
        createTree(-15 - Math.random() * 20, treeSpawnZ - Math.random() * 20);
        createTree(15 + Math.random() * 20, treeSpawnZ - Math.random() * 20);
    }
    
    // Remove old trees behind camera
    scene.children.forEach(child => {
        if (child.position && child.position.z > camera.position.z + 50) {
            scene.remove(child);
        }
    });
