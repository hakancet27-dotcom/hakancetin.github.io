...
let gameState = {
    ...
    speedPenalty: 0,  // Temporary speed penalty (percentage)
    cameraPreset: 1,
    wallTouching: false,
    wallTouchStart: null,
    lastWallDamage: 0,
        ...
        if (obstacle.position.z > 8 && obstacle.position.z < 12) {
            ...
            if (distance < 2) {
                ...
                if (type === 'turbo') {
                    ...
                    gameState.health -= 20;
                    gameState.speedPenalty = 15; // Apply 15% speed penalty
                    playRedSound();
...
function onFaceResults(results) {
    if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
        ...
        if (!gameState.isCalibrated) {
            ...
            gameState.targetSpeed = Math.max(0, Math.min(gameState.maxSpeed * (1 - gameState.speedPenalty / 100), 
            ...
            // Gradually reduce speed penalty
            if (gameState.speedPenalty > 0) {
                gameState.speedPenalty = Math.max(0, gameState.speedPenalty - 0.02); // Reduce by 0.02% per frame (slower recovery for visibility)
            }
