...
function updateGame() {
    ...
    if (car.position.x < -barrierOffset) {
        car.position.x = -barrierOffset;
    } else if (car.position.x > barrierOffset) {
        car.position.x = barrierOffset;
    }

    // Check if car is touching wall
    if (car.position.x <= -barrierOffset || car.position.x >= barrierOffset) {
        if (!gameState.wallTouching) {
            ...
            if (now - gameState.lastWallDamage >= 100) {
                ...
                if (gameState.health < 100 && gameState.health % 20 === 0) {
                    playWallFrictionSound();
                    createWallFrictionParticles(car.position.x <= -barrierOffset ? -barrierOffset : barrierOffset);
                }
                if (gameState.health <= 0) {
                    endGame();
                    return;
