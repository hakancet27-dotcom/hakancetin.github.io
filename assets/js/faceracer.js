...
function updateGame() {
    ...
    if (gameState.nitroActive) {
        currentSpeed = Math.min(350, currentSpeed + 0.5); // Turbo 1 saniyede 350 km/h
    }
...
function updateSpeedometer(currentSpeed) {
    ...
    // Determine color class (updated for 320-350 km/h range)
    let newClass = '';
    if (speed < 200) {
        newClass = 'speed-low';
    } else if (speed < 300) {
        newClass = 'speed-medium';
    } else {
        newClass = 'speed-high';
    }
    
    // Only update DOM if class changed
...
function onFaceResults(results) {
    if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
        ...
        if (!gameState.isCalibrated) {
            ...
            gameState.targetSpeed = Math.max(0, Math.min(320, 
