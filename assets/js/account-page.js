(function(){
  ...
  function button(label,className){
    ...
    item.type='button';
    item.className=className||'member-inline-btn';
  ...
  document.addEventListener('DOMContentLoaded',async function(){
    try{
      ...
      var displayName=metadataName(currentUser);
      var title=document.querySelector('.member-top h1');
      if(title)title.textContent='Hoş geldin, '+displayName;
      ...
      if(headerInfo&&!node('session-status')){
        ...
        activeBadge.id='session-status';
        activeBadge.className='member-session-status';
        activeBadge.textContent='✓ Oturumunuz açık';
        headerInfo.appendChild(activeBadge);
        var back=document.createElement('a');
        back.className='member-btn member-back-link';
        back.href='/haber/';
        back.textContent='Haberlere Dön';
      ...
      if(savedPanel){savedPanel.id='saved';var savedHeading=savedPanel.querySelector('h2');if(savedHeading)savedHeading.textContent='Kaydettiklerim';}
      var savedList=node('saved-list');
      var savedCount=0;

      function updateSavedCount(){
        var count=node('saved-count');
      ...
      async function removeSavedArticle(item,row){
        ...
        updateSavedCount();
        if(savedList&&!savedList.querySelector('.member-saved-row')){
      ...
      function renderSavedItem(item){
        ...
        row.className='member-saved-row';
        var link=document.createElement('a');
        link.href=item.article_url||'#';
        link.textContent=item.article_title||item.article_url||'Haber';
        link.className='member-saved-title';
        var actions=document.createElement('div');
        actions.className='member-saved-actions';
        var open=document.createElement('a');
        open.href=item.article_url||'#';
        open.textContent='Habere Git';
        open.className='member-inline-btn';
        var remove=button('Kaldır','member-inline-btn danger');
        remove.addEventListener('click',async function(){
          remove.disabled=true;
          try{await removeSavedArticle(item,row);}
      ...
      if(savedList){
        ...
        if(!node('saved-message')){
          ...
          savedMessage.id='saved-message';
          savedMessage.className='member-message';
          savedPanel.appendChild(savedMessage);
        }
      }
      ...
      if(notificationPanel){
        ...
        node('pref-mail').checked=prefs.email_daily_digest;
        node('pref-push').checked=prefs.breaking_news_push;
        node('prefs-form').addEventListener('submit',async function(event){
          event.preventDefault();
          prefs.email_daily_digest=!!node('pref-mail').checked;
          prefs.breaking_news_push=!!node('pref-push').checked;
      ...
      if(grid){
        ...
        interestPanel.className='member-panel member-interest-panel';
        interestPanel.id='interests';
        var checkboxes=CATEGORIES.map(function(category){
          return '<label class="member-interest"><input type="checkbox" name="interest" value="'+category.value+'"> '+category.label+'</label>';
        }).join('');
        ...
        prefs.categories.forEach(function(category){
          var checkbox=interestPanel.querySelector('input[value="'+category+'"]');
          if(checkbox)checkbox.checked=true;
        });
        node('interests-form').addEventListener('submit',async function(event){
          event.preventDefault();
          prefs.categories=Array.prototype.slice.call(interestPanel.querySelectorAll('input[name="interest"]:checked')).map(function(input){return input.value;});
          var response=await HaberMember.savePrefs(prefs);
      ...
      if(location.hash){
        window.setTimeout(function(){var target=document.querySelector(location.hash);if(target)target.scrollIntoView({behavior:'smooth',block:'start'});},50);
      }
    }catch(error){
