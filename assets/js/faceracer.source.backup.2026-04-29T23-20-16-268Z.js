...
// Firebase Configuration (Safe - won't block game if fails)
let database = null;
let firebaseInitialized = false;

function initializeFirebase() {
    if (firebaseInitialized) return; // Already initialized
    ...
    try {
        ...
        const firebaseConfig = {
            ...
            storageBucket: "faceracer-leaderboard.appspot.com",
...
// Load leaderboard from Firebase (safe)
function loadLeaderboard() {
    if (!firebaseInitialized || !database) {
        const leaderboardList = document.getElementById('leaderboardList');
        if (leaderboardList) {
            leaderboardList.innerHTML = '<div style="color: #888; text-align: center; padding: 10px;">Leaderboard kullanılamıyor</div>';
        }
        return;
    }
    ...
    leaderboardList.innerHTML = '<div style="color: #888; text-align: center; padding: 10px;">Yükleniyor...</div>';
    ...
    database.ref('leaderboard').orderByChild('score').limitToLast(10).once('value', (snapshot) => {
        ...
        if (!data) {
            leaderboardList.innerHTML = '<div style="color: #888; text-align: center; padding: 10px;">Henüz skor yok</div>';
            return;
        }
        ...
        scores.forEach((entry, index) => {
            ...
            div.style.cssText = 'padding: 8px; border-bottom: 1px solid #444; display: flex; justify-content: space-between; align-items: center;';
            div.innerHTML = `
                <span style="color: ${index < 3 ? '#00ff88' : '#888'}; font-weight: ${index < 3 ? 'bold' : 'normal'};">${index + 1}. ${entry.name}</span>
                <span style="color: #00ff88; font-weight: bold;">${entry.score}</span>
            `;
            leaderboardList.appendChild(div);
        });
    }).catch((error) => {
        console.error('Leaderboard yüklenirken hata:', error);
        leaderboardList.innerHTML = '<div style="color: #ff4444; text-align: center; padding: 10px;">Hata oluştu</div>';
    });
}

// Submit score to Firebase (safe)
function submitScore(score) {
    if (!firebaseInitialized || !database) {
        alert('Leaderboard kullanılamıyor');
        return;
    }
...
function endGame() {
    ...
    calibrationContent.innerHTML = `
        ...
        <button onclick="restartGame()" style="margin-top: 12px; padding: 10px 20px; background: #00ff88; border: none; border-radius: 10px; cursor: pointer; font-size: 0.85rem; font-weight: bold;">
            &#128257; Tekrar Oyna
        </button>
    `;
